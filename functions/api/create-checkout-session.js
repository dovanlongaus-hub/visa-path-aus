...
export async function onRequestPost(context) {
  ...
  try {
    const { planId, durationId, email } = await context.request.json();
    ...
    const totalAud = Math.round(totalVnd / VND_TO_AUD_RATE * 100) / 100;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round(totalAud * 100),
          },
          quantity: 1,
        },
      ],
      ...
      cancel_url: "https://visa.longcare.au/cancel",
